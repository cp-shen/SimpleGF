...
static inline unsigned GetPixelOffset(unsigned col, unsigned row,
        unsigned width, unsigned height, ImageFormat format)
{
    return (row * width + col) * static_cast<int>(format);
}

Image::Image(unsigned width, unsigned height, ImageFormat format,
...
void Image::_set(unsigned width, unsigned height, ImageFormat format,
                  const unsigned char* pixels)
{
    if(width == 0)
...
std::shared_ptr<Image> Image::imageFromFile(const char* filePath) {
    ...
    auto img = std::make_shared<Image> (width, height,
                static_cast<ImageFormat>(channels), pixels);
