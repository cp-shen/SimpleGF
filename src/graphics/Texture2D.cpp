#include <SimpleGF/graphics/Texture2D.h>
...
{
    switch (format) {
    case ImageFormat::Format_Grayscale:
        return GL_LUMINANCE;
    case ImageFormat::Format_GrayscaleAlpha:
        return GL_LUMINANCE_ALPHA;
    case ImageFormat::Format_RGB:
        return GL_RGB;
    case ImageFormat::Format_RGBA:
        return GL_RGBA;
    default:
        throw std::runtime_error("Unrecognised ImageFormat");
    }
}
    ...
    : _width(img.width()), _height(img.height())
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, getTextureFormat(img.format()),
                 (GLsizei)img.width(), (GLsizei)img.height(), 0,
                 getTextureFormat(img.format()), GL_UNSIGNED_BYTE, img.pixelBuffer());
